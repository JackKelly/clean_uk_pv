...
def _(ds):
    ...
    GB_LONGITUDE=list(
        filter(lambda longitude: longitude > 360 +
               GB_LONGITUDE_WEST, ds.longitude.values)
...
def _(GB_LATITUDE, GB_LONGITUDE_EAST, GB_LONGITUDE_WEST, np):
    def lat_lon_to_discrete_index(lat: float, lon: float) -> np.uint16:
        ...
        if not min_lat <= lat <= max_lat:
            raise ValueError(f"`lat` must be between {
                             min_lat} and {max_lat}, not {lat}.")

        # Sanity-check `lon`:
        min_lon=min(GB_LONGITUDE_EAST, GB_LONGITUDE_WEST)
        max_lon=max(GB_LONGITUDE_EAST, GB_LONGITUDE_WEST)
        if not min_lon <= lon <= max_lon:
            raise ValueError(f"`lon` must be between {
                             min_lon} and {max_lon}, not {lon}.")
    ...
    df=(
        ...
        .with_columns(
            h3=plh3.latlng_to_cell(
                "latitude", "longitude", resolution=6, return_dtype=pl.UInt64)
        )
